...
describe("HelloWorld", function () {
  ...
  it("Should execute transferOwnership correctly", async function () {
    const transfer = await helloWorldContract
        .connect(accounts[0])
        .transferOwnership(accounts[1].address);
  ...
  it("Should not allow anyone other than owner to change text", async function () {
    await expect(helloWorldContract
      .connect(accounts[1])
      .setText("Does not matter, it is gonna get reverted.")).to.be.revertedWith("Caller is not the owner");
  });

  it("Should change text correctly", async function () {
    const changedText = "Encode Club!"
    await helloWorldContract
      .setText(changedText);
    
    const text = await helloWorldContract.helloWorld();
    expect(text).to.equal(changedText);
  });
});